...
class RSSGenerator:
        ...
        if episode_number is None:
            if existing_episodes is None:
                ...
                if self._should_use_s3():
                    ...
                    if existing_feed:
                        existing_episodes = self.load_existing_feed(
                            existing_feed)
                    else:
                        existing_episodes = []
                else:
    ...
    def generate_rss_feed(
        ...
        # Channel metadata
        ET.SubElement(
            channel, "title").text = channel_title or self.config.show_name
        ...
        ET.SubElement(
            channel, "generator").text = "The Data Packet RSS Generator"

        # iTunes specific tags
        ET.SubElement(
            channel, "itunes:subtitle").text = "Tech news and insights"
        ...
        if email:
            ...
            ET.SubElement(
                itunes_owner, "itunes:name").text = self.config.show_name
            ET.SubElement(itunes_owner, "itunes:email").text = email

        # iTunes category
        itunes_category = ET.SubElement(
            channel, "itunes:category", text="Technology")
        ...
        if image_url:
            ...
            ET.SubElement(
                image, "title").text = channel_title or self.config.show_name
    ...
    def _add_episode_to_channel(
        ...
        ET.SubElement(item, "pubDate").text = self._format_rfc822_date(
            episode.pub_date)
    ...
    def load_existing_feed(self, rss_content: str) -> List[PodcastEpisode]:
        ...
        try:
            ...
            logger.info(
                f"Loaded {len(episodes)} episodes from existing RSS feed")
            return episodes
        ...
        try:
            ...
            if self._should_use_s3():
                ...
                if s3_result.success:
                    ...
                    logger.info(
                        f"RSS feed updated successfully: {s3_result.s3_url}")
                else:
                    logger.warning(
    ...
    def _download_existing_rss(self) -> Optional[str]:
        ...
        try:
            ...
            logger.info(
                f"No existing RSS feed found in S3 (or error downloading): {e}")
            return None
