...
Updated with better structure and error handling.
"""
...
def format_all_summaries(summaries: List[str]) -> str:
    ...
    return "\n".join(
        [format_segment_summary(i, summary) for i, summary in enumerate(summaries)]
    )


def validate_prompt_variables(prompt: str, **kwargs) -> bool:
    """
    Validate that all required variables are provided for a prompt template.

    Args:
        prompt: The prompt template string
        **kwargs: Variables to be inserted into the prompt
    ...
    missing_variables = variables_in_prompt - provided_variables
